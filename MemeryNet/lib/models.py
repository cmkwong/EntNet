...
class EntNet(nn.Module):
    ...
    def forward(self, E_s, new_story=True):
        ...
        for E in E_s:
            ...
            states = torch.split(self.H, 1 ,1)
            new_states = []
            for j, state in enumerate(states):
                g = nn.Sigmoid()((torch.mm(self.s.t(), state) + torch.mm(self.s.t(), self.W[:,j].view(-1,1))))
                candidate = nn.Sigmoid()(   torch.addmm(self.params['X_b'][:,j].view(-1,1), self.params['X'], state) +
                                            torch.addmm(self.params['Y_b'][:,j].view(-1,1), self.params['Y'], self.W[:,j].view(-1,1)) +
                                            torch.addmm(self.params['Z_b'], self.params['Z'], self.s))
                new_candidate = state + g * candidate
                state_magnitude = torch.abs(torch.norm(new_candidate, p=2, dim=-1, keepdim=True)) + 1e-8
                new_state = new_candidate / state_magnitude
                new_states.append(new_state)
            self.H = torch.cat(new_states, dim=1)
